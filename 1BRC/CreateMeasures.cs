...
public class CreateMeasurements {
    ...
    private record WeatherStation(string Id, double MeanTemperature) {
        ...
        {
            double m = new Random().NextDouble() *10 + MeanTemperature;
    ...
    public void CreateMesurementsFile(string filePath, int size) {
        ...
        using (var ws = File.OpenWrite(filePath))
        {
            for (int i = 0; i < size; i++)
            {
                if (i > 0 && i % 50_000_000 == 0) 
                    Console.WriteLine($"Wrote {i} measurements in {DateTime.UtcNow.Millisecond - start} ms");
        ...
        Console.WriteLine($"Created file with {size} measurements in {DateTime.UtcNow.Millisecond - start} ms");
